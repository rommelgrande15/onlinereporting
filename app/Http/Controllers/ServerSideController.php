...
class ServerSideController extends Controller
{
    ...
    public function getInspectionsOld2(Request $request){
        ...
        if($inspection){
            foreach ($inspection as $ins){
                ...
                if($ins->mrn_no != ""){
                $edit_mrn = route('edit-project-mrn',$ins->mrn_no);
    ...
    //05-21-2021
    public function getInspectionsNew(Request $request){
    ...
    public function getClientInspections(Request $request){
        ...
        $columns = array( 
                            ...
                            5=> 'po',
                            6=> 'inspection_date',
                            7=> 'booked_by',  //added by migz 04-08-21
                            8=> 'status',
                            9=> 'action',
                        );
        ...
        if(empty($request->input('search.value'))){         
            $inspection = DB::table('tic_inspections') 
                    ->join('user_infos','user_infos.user_id','=','tic_inspections.client_book_id') //added by migz 04-08-21
                    ->select('tic_inspections.*', 'user_infos.designation AS booked_by')    //added by migz 04-08-21                           
                    ->orderBy($order,$dir)    
                    ->offset($start)
                    ->limit($limit)                
                    ->get();
        ...
        if($inspection){
            foreach ($inspection as $ins){
                ...
                $nestedData['po'] = "<span title='$po_num'>$cut_po</span>";
                $nestedData['inspection_date'] = date('Y-m-d',strtotime($ins->inspection_date));
                $nestedData['booked_by'] = $ins->booked_by;  //added by migz 04-08-21
               /*  $nestedData['created_at'] = date('Y-m-d',strtotime($ins->created_at)); */
