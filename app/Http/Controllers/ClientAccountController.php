...
class ClientAccountController extends Controller
{
        ...
        if(!empty($inspections))
        {
            foreach ($inspections as $inspection)
            {
                ...
                if( $inspection->mrn_no = null ||  $inspection->mrn_no= 0 ||  $inspection->mrn_no = ""){
                    $multiple_edit_button = "<li class='disabled'><a href='#' title='Edit Order'><small>Edit Multiple</small></a></li>";
                } else {
                ...
                if(!empty($client_book)){
                    $btn_group2 = "<div class='dropdown'><button class='btn btn-xs btn-warning dropdown-toggle' type='button' data-toggle='dropdown'>Action<span class='caret'></span></button><ul class='dropdown-menu'>$edit_button $multiple_edit_button $copy_button $cancel_button $delete_button</ul></div>";
    ...
    public function clientPostNewSupplier(Request $request){
      ...
      /*   $supplier->supplier_state_id = $request['supplier_state_id'];
        ...
        if ($supplier->save()) {
            ...
            foreach ($c_person as $i => $value){
                    $contact[$i] = new SupplierContact();
                if($contact[$i]->factory_contact_person = $request['contact_person'][$i] == 'N/A'){
                    $contact[$i] = new SupplierContact();
                    $contact[$i]->supplier_id = $lastInsertId;
                    $contact[$i]->client_code = $request['client_code'];
                    $contact[$i]->supplier_contact_person = $request['contact_person'][$i];
  ...
  /* supplier account */
    ...
    public function updateSupplier(Request $request){
        ...
        if($request['supplier_account_checked']=='true'){
    ...
    public function getSuppliersListClient(){
        ...
        $clients_contacts_update = DB::table('client_contacts')
                ->where('client_code', $role->client_code)->get();
        $countries = Country::all();
        $user_info = UserInfo::where('user_id',Auth::id())->first();
        $user = User::where('id',$client_id)->first();
        $suppliers = Supplier::where('client_code',$user->client_code)->where('supplier_status',0)->get();
        $suppliersId = Supplier::where('client_code',$user->client_code)->where('supplier_status',0)->first();
        $client_code = $user->client_code;
        // $supplierData = SupplierData::where('supplier_id',$suppliersId->id)->first();
        ...
        $new_post_client = Inspection::where('inspection_type',null)
                                    ...
                                    ->count();
        $new_post_client_sera = Inspection::where('inspection_type','tic_sera')->where('inspection_status','Client Pending')->count();
        return view('pages.client.supplier.index',compact('role','suppliers','user_info','countries','client_code','user','sub_acc','privelege','new_post_client','new_post_client_sera','clients_contacts_update'));
    }
    ...
    public function getSuppliersBook(){
       ...
       return view('pages.client.supplier.supplierbook',compact('role','user_info','countries','suppliers','client_code','user','sub_acc','privelege','new_post_client','new_post_client_sera'));
   }
   ...
   public function getClientSuppliersBookPanel(Request $request){
    ...
    public function getFactoryBySupplier($id){
        ...
        $new_post_client = Inspection::where('inspection_type',null)
                                    ...
                                    ->count();
        $new_post_client_sera = Inspection::where('inspection_type','tic_sera')->where('inspection_status','Client Pending')->count();
        return view('pages.client.factory.index',compact('role','factories','user_info','client_code','req','supplier_id','user','suppliers','sub_acc','privelege','new_post_client','new_post_client_sera','f_count'));
    }
    ...
    public function getTrackInspectionDashboard($id){
		...
		return view('pages.client.trackInspection.index',compact('role','user_info','client_code','client','client_contact','user','id','new_post_client'));    	
    }
    ...
    public function getOneSupplierForFactory($id){
        $supplier = Supplier::find($id); 
        $factory = Factory::where('supplier_id',$id)->where('factory_status',0)->count(); 
        $contacts = SupplierContact::where('supplier_id',$id)->where('supplier_contact_status',0)->get();   
    ...
    public function clientPostNewFactorySupplier(Request $request){
        $factory = new Factory();
        $factory->client_code = $request['client_code'];
        $factory->supplier_id = $request['supplier_id'];
        $factory->factory_name = $request['factory_name'];
    ...
    public function getOneSupplierWithAccount($id){
        $supplierData = SupplierData::where('supplier_id',$id)->first(); 
        if($supplierData = SupplierData::where('supplier_id',$id)->count() == 0){
            $supplier = Supplier::find($id);
            $contacts = SupplierContact::where('supplier_id',$id)->where('supplier_contact_status',0)->get();   
            $withAccount = "false";
            return response()->json([
            ...
            $contacts = SupplierContact::where('supplier_id',$id)->where('supplier_contact_status',0)->get(); 
            $supplierData = SupplierData::where('supplier_id',$id)->first(); 
            $account = UserInfo::where('user_id', $supplierData->user_id)->first();
