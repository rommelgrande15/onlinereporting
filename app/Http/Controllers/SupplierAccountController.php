...
class SupplierAccountController extends Controller
{
    ...
    public function getAccountDashboard(){
        ...
        //05-11-2021
        $supplierClientContact = DB::table('client_contacts')->where('id',$supplierInfo->supplier_client_contact_id)->first();
        $supplierContactName = DB::table('supplier_contacts')->where('id',$supplierInfo->supplier_contact_id)->first(); 

        return view('pages.supplier.accountsettings.index',compact('user_info','user','supplierInfo','supplierClientContact','supplierContactName','supplierContactInfo','client_aql_details','client','supplierData','supplierInfo'));
    ...
    public function getSupplierProjectForm(){
        ...
        $suppFactory = DB::table('factories')->where('supplier_id',$suppliers1->id)
 ...
 //       ]);
    ...
    //    $supplier1 = DB::table('suppliers')->where('id', $supplierData1->supplier_id)->first();
     //   $clientName1 = Client::where('client_code', $supplier1->client_code)->first();
        ...
        return view('pages.supplier.project.index', compact('role','user_info','inspectors','factories','countries','products','inspectors_new','inspectors_two',
    ...
    //migz supplier client save inspection
        ...
        $validator = Validator::make($request->all(), [
         ...
         if($validator->fails()) {
             ...
             try {
                 ...
                 //factory details
                 $inspection->supplier_id = $request['supplier-id'];
    ...
    public function getInspectionLoadingProjectFormEditSupplier($id){
        ...
        $factory_list = Factory::where('supplier_id',$supplierData->id)->where('factory_status',0)->orderBy('factory_name','asc')->pluck('factory_name','id');
        ...
        return view('pages.supplier.edit-project.index_loading',compact('role','user_info','client_id','client_code','client_contact','inspection','get_factory','get_fc',
        'factory_con_list','factory_list','get_cc','user','supplier_list','supplier_con_list','supplier_info','supplier_con_info','sub_acc','privelege','supplierData',
        'supplierInfo','suppFactory','clientName1','supplierContactNew'));
    }
   ...
   public function updateSupplierAccountAccess(Request $request){
    ...
    public function getOneSupplierContactClient($id){
        ...
        $supplier_contact = SupplierContact::where('supplier_id',$supplier->id)->where('supplier_contact_status',0)->get();   
        return response()->json([
            'supplier' => $supplier,
            'contacts' => $contacts,
            'supplier_contact' =>$supplier_contact,
    ...
    public function getOneSupplierClientContact($id){
        $supplier_contact = SupplierContact::where('id',$id)->first();  
            if(User::select('email')->where('email',$supplier_contact->supplier_email)->first()){
    ...
    public function getOneSupplierClientContactUpdate($id){
        $contacts = SupplierContact::where('id',$id)->first();  
            if(User::select('email')->where('email',$contacts->supplier_email)->first()){
                //return response()->json("Email Address Already registered",400);
                return response()->json([
                    'message' => "Email Address Already Registred As A User, Please Edit Email Address Of Client Contact or Add New Contact Client Contact Person"
                ],400);
