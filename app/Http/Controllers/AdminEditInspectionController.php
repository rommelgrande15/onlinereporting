...
class AdminEditInspectionController extends Controller
{
    ...
    public function getInspectionProjectFormEdit($inspection_id){
        ...
        return view('pages.admin.edit-project.index',compact('clients','role','user_info','p_category','inspectors','factories','countries','products','templates','inspectors_new',
        'inspection_details','psi_sub_servie','client_contacts','inspector_info','factory_info','factory_contactlist','factory_contact1','factory_contact2','psi_product',
        'client_contact','contact_person_list','fac_contact_person_list','templates_chinese','currency','client_cost','inspector_cost','client_other_cost_text',
        'client_other_cost_val','ins_other_cost_text','ins_other_cost_val','client_other_cost_array','ins_other_cost_array','other_inspector','inspector_list','new_client_count',
        'new_post_client','new_post_client_sera','fm_accounts'));
    }
    ...
    public function updateInspectionPsiDataFromDraft(Request $request){
        ...
        try {
            ...
            if ($inspection->save()) {
                ...
                // $report_data = Report::where('inspection_id',$inserted_inspection_id)->first();
    ...
    public function saveDraftInspection(Request $request){
        ...
        try {
            ...
            $inspection->memo = $request['memo'];
            $template=$request['template'];
            $template_word=$request['template_word']; //04-28-2021
            // if($template=="" || $request['type_of_project']=="word_project" || $request['type_of_project']=="esprit"){$template=0;}
            //04-28-2021
            if($template=="" || $request['type_of_project']=="esprit"){$template=0;}
            //04-28-2021
            ...
            if ($inspection->save()) {
                ...
                foreach ($products as $i => $value) {
                    ...
                    $prods[$i]->product_name = $request['product_name'][$i];
                    $prods[$i]->product_first_category = $request['order_product_category'][$i];
                    $prods[$i]->product_category = $request['product_sub_category'][$i];
                    $prods[$i]->brand = $request['brand'][$i];
                    $prods[$i]->po_no = $request['po_number'][$i];
                    $prods[$i]->model_no = $request['model_no'][$i];
    ...
    public function saveDraftInspectionCbpi(Request $request){
        ...
        try {
            ...
            $template_word=$request['loading_template_word'];//05-05-2021
    ...
    public function updateDraftInspectionCbpi(Request $request){
        ...
        try {
            ...
            $inspection->supplier_name = $request['loading_supplier_name'];
            $template=$request['loading_template'];
            $template_word=$request['loading_template_word']; //05-28-2021
            if($template==""){$template=0;}
    ...
    public function postCBPIDataFromDraft(Request $request){
        ...
        try{
            ...
            $loading_template_word=$request['loading_template_word'];
            ...
            //05-28-2021
            if($request['project_type_cbpi']=="word_project"){
