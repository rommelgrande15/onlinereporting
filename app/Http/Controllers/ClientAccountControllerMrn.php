...
class ClientAccountControllerMrn extends Controller
{
    ...
    public function getDashboardPanelClient($id){
        ...
        return view('pages.client.dashboard.index',compact('inspections','services','role','user_info','user_manager','product','psiproduct','services_new','inspector_list','services_client','user','sub_acc','privelege','new_post_client','new_post_client_sera'));   
    }
    ...
    public function getDashboardPanelClientDev(Request $request){
        ...
        $new_post_client = Inspection::where('inspection_type',null)->where('inspection_status','Client Pending')->count();
        $new_post_client_sera = Inspection::where('inspection_type','tic_sera')->where('inspection_status','Client Pending')->count();
        return view('pages.client.dashboard.index-dev',compact('user_info','user','new_post_client','new_post_client_sera'));   
    }
    public function searchDashboardSummaryReport(Request $request){
        ...
        ->where('inspections.client_id','J439')
    ...
    public function getDashboardSummaryReport(Request $request){
        ...
        return view('pages.client.dashboard.index-summary',compact('user_info','user','SummaryData','new_post_client','new_post_client_sera'));   
    }
    ...
    public function getDashboardPanelClientServerSide(Request $request){
        ...
        $totalData = DB::table('client_isnpections')
            ->join('inspections','client_isnpections.id','=','inspections.id')
            ->select('client_isnpections.*','inspections.mrn_no')
            ->where('user_id',$client_id)
            ->where('client_isnpections.client_book',$client_book)
            ->count();
        ...
        {            
            ...
            ->select('client_isnpections.*','inspections.mrn_no')
                ...
                ->where(function($query) use ($search) {
                    $query->where('client_isnpections.report_no','LIKE',"%{$search}%")
                        ->orWhere('client_isnpections.client_project_number', 'LIKE',"%{$search}%")
                        ->orWhere('client_isnpections.factory_name', 'LIKE',"%{$search}%")
                        ->orWhere('client_isnpections.product_names', 'LIKE',"%{$search}%")
                        ->orWhere('client_isnpections.model_no', 'LIKE',"%{$search}%")
                        ->orWhere('client_isnpections.manday', 'LIKE',"%{$search}%")
                        ->orWhere('client_isnpections.po_no', 'LIKE',"%{$search}%")
                        ->orWhere('client_isnpections.inspection_status', 'LIKE',"%{$search}%")
                        ->orWhere('client_isnpections.created_at', 'LIKE',"%{$search}%");
                })->offset($start)
                ->limit($limit)
                ->orderBy($order,$dir)
                ->get();
        ...
        if(!empty($inspections))
        {
            foreach ($inspections as $inspection)
            {
                ...
                $multiple_edit_button = "<li><a href='".route('edit-project-client-mrn',$inspection->id)."' title='Edit Order Multiple'><small>Multiple Order Edit</small></a></li>";

                if( $inspection->mrn_no != null ||  $inspection->mrn_no != 0 ||  $inspection->mrn_no != ""){
                    if(!empty($client_book)){
                        $btn_group2 = "<div class='dropdown'><button class='btn btn-xs btn-warning dropdown-toggle' type='button' data-toggle='dropdown'>Action<span class='caret'></span></button><ul class='dropdown-menu'>$multiple_edit_button $copy_button $cancel_button $delete_button</ul></div>";
    ...
    public function getDashboardPanelClientByBooking(){
        ...
        return view('pages.client.dashboardBook.index',compact('inspections','services','role','user_info','user_manager','product','psiproduct','services_new','inspector_list','services_client','user','sub_acc','privelege','new_post_client','new_post_client_sera'));   
    }
    ...
    public function getProductDashboard(){
		...
		return view('pages.client.product.index',compact('role','user_info','client_code','products','user','p_category','sub_acc','privelege','new_post_client','new_post_client_sera'));    	
    }
    ...
    public function getInspectionProjectForm(){
        ...
        return view('pages.client.project.index_mrn',compact('role','user_info','inspectors','factories','countries','products','inspectors_new','inspectors_two','client_id','client_code','ref_num','client_contact','client_aql_detail','client_aql_minors','client_aql_majors','normal','special','aql_major','user','suppliers','units','categories','sub_categories','p_category','services','sub_acc','privelege','new_post_client','new_post_client_sera'));
    }
    ...
    public function postInspectionData(Request $request){
        ...
        $validator = Validator::make($request->all(), [
            ...
            // 'file' => 'required|string|max:150|alpha_num'
        ]);
        ...
        if($validator->fails()) {
            ...
            try {
                ...
                foreach($new_data as $items => $item){
                    ...
                    // $validateFileName = Validator::make($request->all(), [
                    //     'file' => 'required|string|max:150|alpha_num'
                    // ]);
                    // if($validateFileName->fails()) {
                    //     return response()->json([
  ...
  /* supplier account */
            ...
            if($request['checkedBtn']===true || $request['checkedBtn']==='true'){
                ...
                if ($user->save()) {
                     ...
                     if ($UserInfo->save()) {
                        ...
                        $SupplierData->email_reciever=$request['email_reciever'];
                        ...
                        Mail::send('email.new_supplier_informations',$data, function($message) use ($data){
                            if($data['email']){
                                $message->to($data['email']);
                                $message->subject($data['account_name']);
                            }else{
                                $message->bcc('it-support@t-i-c.asia','IT Support');
                                $message->subject($data['account_name']);
                            }     
    ...
    public function getFactoryListClient(){
        ...
        $new_post_client = Inspection::where('inspection_type',null)->where('inspection_status','Client Pending')->count();
        $new_post_client_sera = Inspection::where('inspection_type','tic_sera')->where('inspection_status','Client Pending')->count();
        return view('pages.client.factory.index',compact('role','factories','user_info','countries','client_code','req','user','new_post_client','new_post_client_sera'));
    }
    public function getSuppliersListClient(){
        ...
        $new_post_client = Inspection::where('inspection_type',null)->where('inspection_status','Client Pending')->count();
        $new_post_client_sera = Inspection::where('inspection_type','tic_sera')->where('inspection_status','Client Pending')->count();
        return view('pages.client.supplier.index',compact('role','suppliers','user_info','countries','client_code','user','sub_acc','privelege','new_post_client','new_post_client_sera'));
    }
    ...
    public function getSuppliersBook(){
       ...
       return view('pages.client.supplier.supplierbook',compact('role','suppliers','user_info','countries','client_code','user','sub_acc','privelege','new_post_client','new_post_client_sera'));
   }
   ...
   public function getClientSuppliersBookPanel(Request $request){
    ...
    $totalData = DB::table('inspections')
        ->leftJoin('users', 'users.id', '=', 'inspections.client_book_id')
        ->leftJoin('clients', 'clients.id', '=', 'users.group_id')
        ->select('clients.*','inspections.*')
    ...
    $inspections = DB::table('inspections')
        ...
        ->leftJoin('users', 'users.id', '=', 'inspections.client_book_id')
        ->leftJoin('clients', 'clients.id', '=', 'users.group_id')
        ->select(DB::raw('group_concat(p_s_i_products.model_no) as pmodel, group_concat(p_s_i_products.product_name) as pname, group_concat(p_s_i_products.po_no) as pno'),
                        'inspections.*','factories.factory_name', 'inspections.manday','inspections.inspection_status', 'inspections.created_at', 'inspections.service' )
        ->where('clients.user_id', Auth::id())
        ->where('supplier_book', "true")
        ->where('inspection_status', '!=', "Deleted")
    ...
    if(!empty($inspections))
    {
        foreach ($inspections as $inspection)
        {
            ...
            $nestedData['created_at'] = substr($inspection->created_at,0,10);
            $nestedData['view_edit'] = $track_view;
            $mrn_edit = "<li><a href='".route('edit-project-client-mrn',$inspection->id)."' title='Edit Multiple Order'><small>Edit Multiple Order</small></a></li>";
            if($inspection->mrn_no != "" || $inspection->mrn_no != null){
                if(!empty($supplier_book)){
                    $btn_group2 = "<div class='dropdown'><button class='btn btn-xs btn-warning dropdown-toggle' type='button' data-toggle='dropdown'>Action<span class='caret'></span></button><ul class='dropdown-menu'>$mrn_edit $copy_button $cancel_button $delete_button</ul></div>";
                    $nestedData['edit_cancel'] = $btn_group2;
                }
    ...
    public function getFactoryBySupplier($id){
        ...
        $new_post_client = Inspection::where('inspection_type',null)->where('inspection_status','Client Pending')->count();
        $new_post_client_sera = Inspection::where('inspection_type','tic_sera')->where('inspection_status','Client Pending')->count();
        return view('pages.client.factory.index',compact('role','factories','user_info','client_code','req','supplier_id','user','suppliers','sub_acc','privelege','new_post_client','new_post_client_sera'));
    }
    ...
    public function getCreateAccountForm(){
        ...
        $new_post_client = Inspection::where('inspection_type',null)->where('inspection_status','Client Pending')->count();
        $new_post_client_sera = Inspection::where('inspection_type','tic_sera')->where('inspection_status','Client Pending')->count();
        return view('pages.client.createAccount.index',compact('countries','new_post_client','new_post_client_sera'));
    }
    ...
    public function getCreateAccountFormNew(){
        ...
        $new_post_client_sera = Inspection::where('inspection_type','tic_sera')->where('inspection_status','Client Pending')->count();
        $countries = Country::pluck('nicename','phonecode');
        return view('pages.client.createAccount.new_register',compact('countries','new_post_client','new_post_client_sera'));
    }
    ...
    public function activateAccount($code){
        ...
        return view('pages.client.createAccount.activateaccount','new_post_client','new_post_client_sera');
    ...
    public function getAccountDashboard(){
        ...
        return view('pages.client.accountsettings.index',compact('role','user_info','client_code','client','client_contact','user','client_aql_detail','client_aql_minors','client_aql_majors','normal','special','aql_major','sub_acc','privelege','new_post_client','new_post_client_sera')); 
    }
    ...
    public function getAccountDashboardNew(){
        ...
        $new_post_client = Inspection::where('inspection_type',null)->where('inspection_status','Client Pending')->count();
        $new_post_client_sera = Inspection::where('inspection_type','tic_sera')->where('inspection_status','Client Pending')->count();
        return view('pages.client.accountsettings.index2',compact('role','user_info','client_code','client','client_contact','user','client_aql_detail','client_aql_minors','client_aql_majors','normal','special','aql_major','new_post_client','new_post_client_sera')); 
    }
