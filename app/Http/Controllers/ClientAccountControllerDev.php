...
class ClientAccountControllerDev extends Controller
{
    ...
    public function getInspectionProjectForm(){
        ...
        return view('pages.client.project.index_dev',compact('role','user_info','inspectors','factories','countries','products','inspectors_new','inspectors_two','client_id','client_code','ref_num','client_contact','client_aql_detail','client_aql_minors','client_aql_majors','normal','special','$aql_major','user','suppliers','units','categories','sub_categories','p_category'));
    }
                ...
                if ($report->save()) {
                    ...
                    $data = ['report_number' =>  $request['reference_number'],
                        ...
                        Mail::send('email.book_from_client',$data, function($message) use ($data){
                            ...
                            $message->cc('report@t-i-c.asia','Report');
    ...
    public function getSavedProductSubCategory(Request $request){
        ...
        $saved_sub_categories  = SavedProductSubCategories::where('category',$id)->get();
        return response()->json([
    ...
    public function registerClient(Request $request){
       /*  $client = new Client();
        ...
        $count=0;
        $count = Client::where('Company_Email','=',$email)->count();
        if($count<=0){
            ...
            $data = ['client_rep_email' =>  $request['username'],
                ...
                Mail::send('email.verify',$data, function($message) use ($data){
                   ...
                   /* $message->cc('it-support@t-i-c.asia','IT Support');
                    $message->cc('gregor@t-i-c.asia','Gregor'); 
                    $message->cc('booking@t-i-c.asia','Booking'); */            
                    $message->subject("Account Registration - The Inspection Company Online Booking");                                              
                });               
