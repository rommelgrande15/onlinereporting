...
class AdminAccountControllerDevMR extends Controller
{
    ...
    public function getInspectionProjectForm(){
        ...
        $client_code = $client->user_id;

        date_default_timezone_set('UTC');
        $year=date("y");
        $month=date("m");
        $Inspection = Inspection::where('created_by',$client_code)->get();
        ...
        return view('pages.admin.project.index_mrn',compact('clients','role','ref_num','user_info','p_category','inspectors','factories','countries','products','templates',
        'templates_chinese','inspectors_new','inspectors_two','currency','new_client_count','new_post_client','new_post_client_sera','fm_accounts'));
    }
    ...
    public function deleteAttachments(Request $request){
        ...
        $mrn_inspection_id = $request['inspection_id']; //05-31-2021
        ...
        $cond=['file_name'=>$request['file_name']]; //05-31-2021
    ...
    public function postInspectionData(Request $request){
        ...
        try {
            ...
            foreach($new_data as $items => $item){
                ...
                if ($inspection->save()) {
                    $mrn_inspection_id[]=$inspection->id;
                ...
                foreach($multiple_products as $product){
                    ...
                    $product_saved = Product::where('product_name',$product['product_name'])->first();
                    //$product_saved = Product::find($request['product_name'][$i]);
