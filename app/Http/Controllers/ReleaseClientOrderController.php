...
class ReleaseClientOrderController extends Controller
{
    public function getInspectionProjectFormClientOrder($inspection_id){
        ...
        return view('pages.admin.release-order.index_psi',compact('clients','role','user_info','inspectors','templates','inspection_details','client_contacts',
        ...
        try {
            ...
            if ($inspection->save()) {
                ...
                $client = DB::table('clients')
                        ->join('user_infos','clients.user_id','=','user_infos.user_id')
                        ->select('user_infos.name','clients.*')
                        ->where('clients.client_code',$request['client'])
                        ->first();
                ...
                if ($report->save()) {
                    ...
                    if($request['type_of_project']=='word_project' || $request['type_of_project']=="esprit"){                     
                        ...
                        Mail::send('email.manual_release_order',$data, function($message) use ($data,$files_uploaded ){
                            ...
                            // $message->cc('2891400188@qq.com');
                            $message->to('miguelbuojr@gmail.com');
                            $message->cc($data['second_inspector_email']);
                            if(!empty($data['second_inspector_email'])){
                                foreach ($data['second_inspector_email'] as $sec_email) {
                                    $message->cc($sec_email);
                                }
