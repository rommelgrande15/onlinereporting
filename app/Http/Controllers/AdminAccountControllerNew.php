...
class AdminAccountController extends Controller
{
    ...
    public function getDashboardPanel($id){
		...
		return view('pages.admin.dashboard.index',compact('inspections','services','role','user_info','user_manager','product','psiproduct','services_new','inspector_list'));    	
    }
    ...
    public function getClientBookingList(){
		...
		return view('pages.admin.client-booking.index',compact('inspections','services','role','user_info','user_manager','product','psiproduct','services_new','inspector_list'));    	
    }
    ...
    public function getInspectionProjectForm(){
        ...
        return view('pages.admin.project.index',compact('clients','role','user_info','inspectors','factories','countries','products','templates','templates_chinese','inspectors_new','files_blank_report','inspectors_two','currency'));
    }
    ...
    public function getInspectionProjectFormEdit($inspection_id){
        ...
        $inspection_details = DB::table('inspections')
                    ...
                    ->join('factory_contacts','inspections.factory_contact_person', '=', 'factory_contacts.id')
                    ->join('p_s_i_products','inspections.id', '=', 'p_s_i_products.inspection_id')
                    ->select('inspections.*','inspections.factory_contact_person AS fac_con_per','inspections.contact_person AS con_per','inspections.id AS inspec_id','clients.client_name','users.*','user_infos.*','client_contacts.*','factories.*','factory_contacts.*','p_s_i_products.*')
        ...
        $factory_contact1 = FactoryContact::where('id',$inspection_details->fac_con_per)->first();
        $factory_contact2 = FactoryContact::where('id',$inspection_details->factory_contact_person2)->first();

        $psi_product = PSIProduct::where('inspection_id',$inspection_id)->get();
        $client_contact = ClientContact::all();

        $factory_contactlist = FactoryContact::where('factory_id',$inspection_details->factory)->get();
    ...
    public function getInspectionProjectFormEditCbpi($inspection_id){
        ...
        $inspection_details = DB::table('inspections')
                    ...
                    ->join('factories','inspections.factory', '=', 'factories.id')
                    ->join('factory_contacts','inspections.factory_contact_person', '=', 'factory_contacts.id')
                    ->select('inspections.*','inspections.factory_contact_person AS fac_con_per','inspections.contact_person AS con_per','inspections.id AS inspec_id','inspections.supplier_name AS supplier','clients.client_name','users.*','user_infos.*','client_contacts.*','factories.*','factory_contacts.*')
                    ->where('inspections.id',$inspection_id)                    
                    ->first(); 
        $other_inspector=explode(',',$inspection_details->secondary_inspector_id);
    ...
    public function shortPublishInspection(Request $request){
        try {
            ...
            if ($inspection->save()) {
                ...
                $factory_contact = FactoryContact::find($insp_details->factory_contact_person);
                //$factory_contact_sec = FactoryContact::find($fac_contact2);
                ...
                $product_subject = "";
                    ...
                    if($fac_contact2=="" || $fac_contact2=="N/A" || $fac_contact2=="0" || $fac_contact2==0 || $fac_contact2==null){
                        ...
                        foreach($fac_sec_ex as $key => $fid){
                            $fac_sec = FactoryContact::where('id',$fid)->first();
    ...
    public function shortPublishInspectionCbpi(Request $request){
        ...
        if ($inspection->save()) {
            ...
            $fac_contact2=$insp_info->factory_contact_person2;
                ...
                $factory_contact = FactoryContact::find($insp_info->factory_contact_person);
                //$factory_contact_sec = FactoryContact::find($fac_con2);
    ...
    public function getEditProjectDetails($id){
        ...
        $inspection = DB::table('inspections')
                    ...
                    ->select('inspections.*','clients.client_name','users.*','user_infos.*','client_contacts.*','factories.*','factory_contacts.*','p_s_i_products.*','products.*')
            ...
            if ($report->save()) {
                ...
                if($request['project_type_site']=='word_project'){
                    ...
                    $data = ['report_number' =>  $request['site_reference_number'],
                        ...
                        'client_number'=>$request['client_project_number_site'],
    ...
    public function postInspectionDataFromDraft(Request $request){
        ...
        try {
            ...
            if ($inspection->save()) {
                ...
                if($request['is_new_product_added']==0){
                ...
                if ($report->save()) {
                    ...
                    $factory_contact = FactoryContact::find($request['factory_contact_person']);
                    $factory_contact_sec = FactoryContact::find($request['factory_contact_person2_psi']);
                    $psi_product_list = PSIProduct::where('inspection_id',$inserted_inspection_id)->get();
                    $product_list = Product::all();
        ...
        try {
            ...
            if ($inspection->save()) {
                ...
                foreach ($products as $i => $value) {
                    ...
                    array_push($prod_name,$request['product_name']);
    ...
    public function saveDraftInspection(Request $request){
       ...
       /*      $blank_report_type=$request['blank_report_type'];
            if($blank_report_type=="" || $request['type_of_project']=="app_project"){$blank_report_type="N/A";}

            $word_template=$request['word_template'];
            if($word_template=="" || $request['type_of_project']=="app_project"){$word_template="N/A";}

            $inspection->report_type = $blank_report_type;
            $inspection->word_template = $word_template; */
            ...
            if ($inspection->save()) {
               ...
               $inspectorCost->save();
                ...
                if ($report->save()) {
                    ...
                    $factory_contact = FactoryContact::find($request['loading_factory_contact_person']);
    ...
    public function postCBPIData(Request $request){
        ...
        if ($inspection->save()) {
            ...
            if ($report->save()) {
                ...
                $factory = Factory::find($request['loading_factory']);
                $factory_contact = FactoryContact::find($request['loading_factory_contact_person']);
                $factory_contact_sec = FactoryContact::find($request['factory_contact_person2_cbpi']);
    ...
    public function getProjectDetailsNew($id){
        ...
        foreach($factory_contact as $contact){
            $data = FactoryContact::where('id',$contact)->first();
            array_push($factory_contact_list,$data);
        }

        $factory_contact1 = FactoryContact::where('id',$inspection_new->factory_contact_person)->first();
    ...
    public function getProjectDetailsCbpiNew($id){
        ...
        $inspection = DB::table('inspections')
                    ...
                    ->select('inspections.*','clients.client_name','users.*','user_infos.*','client_contacts.*','factories.*','factory_contacts.*')
                    ->where('inspections.id',$id)
    ...
    public function getTemplateForm() {
        ...
        return view('pages.admin.templates.index', compact('clients','role','user_info','inspectors','factories','countries','products','templates'));
    }
