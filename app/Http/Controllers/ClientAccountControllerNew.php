...
class ClientAccountControllerNew extends Controller
{
    ...
    public function getDashboardPanelClient($id){
		...
		return view('pages.client.dashboard.index',compact('inspections','services','role','user_info','user_manager','product','psiproduct','services_new','inspector_list','services_client'));    	
    }
    ...
    public function getProductDashboard(){
		...
		return view('pages.client.product.index',compact('role','user_info','client_code','products'));    	
    }
    ...
    public function getInspectionProjectForm(){
        ...
        $client_aql_minors = ClientAqlMinor::all();
        $client_aql_majors = ClientAqlMajor::all();

        return view('pages.client.project.index',compact('role','user_info','inspectors','factories','countries','products','inspectors_new','inspectors_two','client_id','client_code','ref_num','client_contact','client_aql_detail','client_aql_minors','client_aql_majors'));
    }
    ...
    public function getInspectionProjectFormEdit($id){
        ...
        return view('pages.client.edit-project.index',compact('role','user_info','products','client_id','client_code','client_contact','client_aql_detail','client_aql_minors','client_aql_majors','inspection','get_factory','get_fc','factory_con_list','factory_list','get_cc'));
    }
        ...
        try {
            ...
            if ($inspection->save()) {
                ...
                foreach ($request->file('file') as $file) {
                    ...
                    /* $doc->file_size = $file->getSize(); jesser need to back again*/
                    $doc->file_size = 56716;
    ...
    public function clientUpdateContact(Request $request){
        ...
        $client =ClientContact::find($request['contact_id']);
        $client->client_code = $request['client_code'];
        $client->contact_person = $request['contact_person'];
        $client->email_address = $request['contact_person_email'];
        $client->contact_number = $request['contact_person_number'];
    ...
    public function getFactoryListClient(){
        ...
        $client_code=$user_info->client_code;
        return view('pages.client.factory.index',compact('role','factories','user_info','countries','client_code'));
    }
    ...
    public function getAccountDashboard(){
		...
		return view('pages.client.accountsettings.index',compact('role','user_info','client_code','client','client_contact','user','client_aql_detail','client_aql_minors','client_aql_majors'));    	
    }
    ...
    public function updateClientInfo(Request $request){
        ...
        //Update AQL Details
        }  else if($is_request=='update_aql'){
            $client_aql_detail = ClientAqlDetail::find($request['client_id']);
            $client_aql_detail->normal_level = $request['aql_normal_level'];
            $client_aql_detail->special_level = $request['aql_special_level'];
            $client_aql_detail->aql_major = $request['aql_major'];
            $client_aql_detail->aql_minor = $request['aql_minor'];
            if ($client_aql_detail->save()) {
                return response()->json([
                    'message' => 'OK',
                ],200);
            }
    ...
    /*public function updateAql(Request $request){
        $this->validate($request,array(
            'aql_normal_level'=>'required',
            'aql_special_level'=>'required',
            'aql_major'=>'required',
            'aql_minor'=>'required'
        ));
        ...
        ClientAqlDetail::where('client_id', Auth::id())->update($data);
        return redirect('/account-settings');
    }*/
    
    public function updateAql(Request $request){
        $client_aql_detail = ClientAqlDetail::where('client_id', Auth::id());
        $client_aql_detail->aql_normal_level = $request['aql_normal_level'];
        $client_aql_detail->aql_special_level = $request['aql_special_level'];
        $client_aql_detail->aql_minor = $request['aql_minor'];
        $client_aql_detail->aql_major = $request['aql_major'];
        if($client_aql_detail){
            if ($client_aql_detail->update()) {
                return response()->json([
    ...
    public function updateCompanyDetails(Request $request){
        ...
        $data = array(
			...
			'bldg_number' => $request->input('bldg_number')
		);
