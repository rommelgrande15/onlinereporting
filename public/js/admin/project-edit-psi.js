$(document).ready(function () {
    ...
    $('#btn_product').click(function () {
        ...
        //     // $('.clone-inputs-edit:first').clone().find("input:text").val("").end().appendTo('.product-clone-edit');
        ...
        //     // $('.clone-inputs-edit:last').append('<div class="row"><div class="col-md-4">' +
        //     //     '<div class="form-group">' +
        ...
        if (add_count_null >= 1) {
            ...
            content_clone.find(".product_name").addClass('new_product_name');
            content_clone.find(".product_name").val('');
            //category
            content_clone.find(".pcat").removeClass('s_pcat');
            content_clone.find(".pcat").addClass('new_product_category');
            content_clone.find(".pcat").val('');
            //sub category
            content_clone.find(".psubcat").removeClass('s_scat');
            content_clone.find(".psubcat").addClass('new_product_sub_category');
    ...
    $('#btn_product_edit').click(function () {
        ...
        // var content_clone = $('.clone-inputs-edit:first').clone();
        // //content_clone.find("input").val("");
        // content_clone.find(".form-control").val("");
        // content_clone.appendTo('.product-clone-edit');
    ...
    //added 04-27-2021
    $('body').on('change', '.pcat_clone', function() {
        var dis = this;
        //dis.closest('.clone-inputs').find('.col-view-att').hide();
        $(dis).closest('.clone-inputs').find('.psubcat_clone').empty();
        $(dis).closest('.clone-inputs').find('.psubcat_clone').append('<option value="">Select Sub-product Category</option>');
        var cat_val = $(dis).val();
        console.log(cat_val);
        getSubCategory1(cat_val, dis);
        if (cat_val == 'Others' || cat_val == 'others') {
            $(this).closest('.div_category').find('.modalCategory').modal('show');
        }
    });
    ...
    //added 04-27-2021
    $('body').on('click', '.btn-add-sub-cat-modal-edit-clone', function() {
        var dis = this;
        var pval = $(dis).closest('.clone-inputs').find('.pcat_clone').val();
        if (pval == '' || pval == null || pval == 'Others') {
            swal({
                title: "Warning!",
                text: "Please select product category first",
...
//added 04-27-2021
function getSubCategory1(cat_val, dis) {
    var sub_cat_arr = [];
    $.ajax({
        url: '/get-saved-sub-category',
        type: 'POST',
        ...
        success: function(response) {
            ...
            sub_cat_arr.sort();
            $.each(sub_cat_arr, function(i, element) {
                $(dis).closest('.clone-inputs').find('.psubcat_clone').append('<option value="' + element + '">' + element + '</option>');
            });
            $(dis).closest('.clone-inputs').find('.psubcat_clone').append('<option value="Others">Others</option>');
        }
    });
}
//08-02-2021
function getFMAccount(){
    if($("#fm_inspector option:selected").val() == null || $("#fm_inspector option:selected" ).val() == ''){

    }else{
        var tis = $("#fm_inspector option:selected" ).index() - 1;
        $.ajax({
            url: '/get-inspector-fm-account/' + tis,
            type: 'GET',
            success: function (response) {
...
function changeProjectType(type) {
    ...
    if (type == 'word') {
        ...
        $('#div_template_word').show(); //04-29-2021
