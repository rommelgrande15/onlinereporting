$(document).ready(function() {
    ...
    $('#btn_product_copy').click(function() {
    ...
    $('body').on('click', '.btn-rm-copy-product', function() {
        var count = $('.clone-inputs-edit').length;
        console.log('Count of products: ' + count);
        if (count == 1) {
            $(this).closest('.clone-inputs-edit').find('.psi_required').val("");
        } else {
            $(this).closest('.clone-inputs-edit').remove();
        }
        // var count_prod = $('.clone-inputs-edit').length;
        // var dis_btn = this;
        // var id = $(this).data('id');
    ...
    $('body').on('click', '.draft_confirm_aql', function() {
        var dis = $(this);
        dis.closest('.clone-inputs-edit').find('.edit_qty').val(dis.closest('.AQLModal').find('.new_aql_qty').val());
        dis.closest('.clone-inputs-edit').find('.AQLModal').modal('hide');
    ...
    $('body').on('change', '.fm_inspector', function () {
     ...
     //08-02-2021
     $('body').on('change', '.fm_inspector_cbpi_copy', function () {
        ...
        if($(this).find( "option:selected" ).val() == null || $(this).find( "option:selected" ).val() == ''){
            ...
            $.ajax({
                ...
                success: function (response) {
                    console.log(response);
                    $('#fm_username_cbpi_copy').val(response.username);
                    $('#fm_password_cbpi_copy').val(response.password);
                }
            });
        }
...
function getFMAccountCopy(){
    if($("#fm_inspector_cbpi_copy option:selected").val() == null || $("#fm_inspector_cbpi_copy option:selected" ).val() == ''){

    }else{
        var tis = $("#fm_inspector_cbpi_copy option:selected" ).index() - 1;
        $.ajax({
            url: '/get-inspector-fm-account/' + tis,
            type: 'GET',
            success: function (response) {
...
function changeProjectType(type) {
    ...
    if (type == 'word') {
        ...
        $('#div_template').show();
        $('#div_template_word').hide(); //04-29-2021
        $('#blank_report').hide();
...
function changeProjectTypeCbpi(type) {
    ...
    if (type == 'word') {
        ...
        $('#div_template_word_cbpi').show(); //05-28-2021
        ...
        if (service == 'site_visit') {
            ...
            $('#div_template_cbpi').hide();
            $('#div_template_word_cbpi').hide(); //05-28-2021
            $('#loading_template').removeClass("cli_required");
        } else {
