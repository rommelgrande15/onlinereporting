$(document).ready(function () {
    ...
    $('body').on('change', '.fm_inspector_cbpi', function () {
        ...
        if($(this).find( "option:selected" ).val() == null || $(this).find( "option:selected" ).val() == ''){
            ...
            $.ajax({
                ...
                success: function (response) {
                    console.log(response);
                    $('#fm_username_cbpi').val(response.username);
                    $('#fm_password_cbpi').val(response.password);
                }
            });
        }
        
    });
    $('body').on('change', '.fm_inspector_cbpi_edit', function () {
        ...
        if($(this).find( "option:selected" ).val() == null || $(this).find( "option:selected" ).val() == ''){
            ...
            $.ajax({
                ...
                success: function (response) {
                    console.log(response);
                    $('#fm_username_cbpi_edit').val(response.username);
                    $('#fm_password_cbpi_edit').val(response.password);
                }
            });
        }
...
function getFMAccount(){
    if($("#fm_inspector_cbpi_edit option:selected").val() == null || $("#fm_inspector_cbpi_edit option:selected" ).val() == ''){

    }else{
        var tis = $("#fm_inspector_cbpi_edit option:selected" ).index() - 1;
        $.ajax({
            url: '/get-inspector-fm-account/' + tis,
            type: 'GET',
            success: function (response) {
...
function changeProjectTypeCbpi(type) {
    ...
    if (type == 'word') {
        ...
        $('#div_template_word_cbpi').show(); //05-05-2021
        ...
        if (service == 'site_visit') {
            ...
            $('#div_template_cbpi').hide();
            $('#div_template_word_cbpi').hide();//05-05-2021
            $('#loading_template').removeClass("cli_required");
        } else {
