$(document).ready(function() {
    ...
    var myDZ = new Dropzone("div.file_upload_psi", {
        ...
        parallelUploads: 100,
        maxFiles: 100,
        acceptedFiles: 'application/pdf, .doc, .docx, .pub, .jpeg, .jpg, .png, .gif, .xls, .xlsb, .xlsx, .xlsm, .ppt, .pptx, .DOC, .DOCX, .PUB, .JPEG, .JPG, .PNG, .GIF, .XLS, .XLSB, .XLSX, .XLSM, .PPT, .PPTX, .tif, .TIF, .ai, .AI',
        ...
        init: function() {
                ...
                this.on("sendingmultiple", function(data, xhr, formData) {
                    ...
                    $('.order_product_category').each(function(i, obj) {
                        var val = $(this).find('option:selected').text();
                        formData.append('order_product_category[]', val);
                    });
                    $('.order_product_sub_category').each(function(i, obj) {
                        var val = $(this).val();
...
function savePsiAsDraft() {
    ...
    var fm_username = $('#fm_username').val();
    var fm_password = $('#fm_password').val();
    var fm_inspector = $('#fm_inspector').val();
    ...
    $('.p_unit').each(function(i, obj) {
        var val = $(this).val();
        form_data.append('p_unit[]', val);
    });
    $('.prod_addtl_info').each(function(i, obj) {
        var val = $(this).val();
        form_data.append('prod_addtl_info[]', val);
    });
    ...
    formData.append("fm_username", fm_username);
    formData.append("fm_password", fm_password);
    formData.append("fm_inspector", fm_inspector);
