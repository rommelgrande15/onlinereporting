        ...
        .on('click', '.btn_view_project', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.inspector_cost.currency === 'usd') {
                        var currency = '$';
                        var currencyWord = 'US Dollar';
                    } else if (response.inspector_cost.currency === 'euro') {
                        var currency = '€';
                        var currencyWord = 'Euro';
                    } else if (response.inspector_cost.currency === 'gbp') {
                        var currency = '£';
                        var currencyWord = 'British Pound';
                    } else if (response.inspector_cost.currency === 'cny') {
                        var currency = '¥';
                        var currencyWord = 'Chinese Yuan Renminbi';
                    } else if (response.inspector_cost.currency === 'myr') {
                        var currency = 'RM';
                        var currencyWord = 'Malaysian Ringgit';
                    } else if (response.inspector_cost.currency === 'inr') {
                        var currency = '₹';
                        var currencyWord = 'Indian Rupee';
                    } else {
                        var currency = '$';
                        var currencyWord = 'US Dollar';
                    }
                    if (response.inspector_cost.other_cost_text == null) {
                        var other_cost_text = 'None';
                    } else {
                        var other_cost_text = response.inspector_cost.other_cost_text;
                    }
                    if (response.inspector_cost.other_cost_value == null) {
                        var other_cost_value = 0;
                    } else {
                        var other_cost_value = parseInt(response.inspector_cost.other_cost_value);
                    }
                    var manday_cost = parseInt(response.inspection.manday)
                    var total_cost_inspection = response.inspector_cost.md_charges + response.inspector_cost.travel_cost + response.inspector_cost.hotel_cost + response.inspector_cost.ot_cost + other_cost_value;
                    var total_manday_cost = manday_cost * total_cost_inspection;
                    //inspector_cost for the project
                    $('#table_view_project > tbody:last-child').append('<tr style="background-color:lightgrey" class="proj_added_row">' +
                        '<th colspan="4"><h4>6. Inspection Cost </h4></th > ' +
                        '</tr>' +
                        '<tr class="proj_added_row">' +
                        '<th>Inspection Status: ' + response.inspection.inspection_status + '</th>' +
                        '<th>Manday = ' + response.inspection.manday + '</th>' +
                        '<th>Currency: ' + currencyWord + '</th>' +
                        '<th> Amount </th>' +
                        '</tr>' +
                        '<tr class="proj_added_row">' +
                        '<th>MD Cost :</th>' +
                        '<th></th>' +
                        '<th></th>' +
                        '<td>' + currency + ' ' + response.inspector_cost.md_charges + '</td>' +
                        '</tr>' +
                        '<tr class="proj_added_row">' +
                        '<th>Travel Cost :</th>' +
                        '<th></th>' +
                        '<th></th>' +
                        '<td>' + currency + ' ' + response.inspector_cost.travel_cost + '</td>' +
                        '</tr>' +
                        '<tr class="proj_added_row">' +
                        '<th>Hotel Cost :</th>' +
                        '<th></th>' +
                        '<th></th>' +
                        '<td>' + currency + ' ' + response.inspector_cost.hotel_cost + '</td>' +
                        '</tr>' +
                        '<tr class="proj_added_row">' +
                        '<th>OT Cost :</th>' +
                        '<th></th>' +
                        '<th></th>' +
                        '<td>' + currency + ' ' + response.inspector_cost.ot_cost + '</td>' +
                        '</tr>' +
                        '<tr class="proj_added_row">' +
                        '<th>Other Cost: </th>' +
                        '<td>' + other_cost_text + '</th>' +
                        '<th></th>' +
                        '<td>' + currency + ' ' + other_cost_value + '</td>' +
                        '</tr>' +
                        '<tr class="proj_added_row">' +
                        '<th>Total MD Charge:</th>' +
                        '<td>' + currency + ' ' + response.inspector_cost.md_charges + '</td>' +
                        '<th>Total Cost:</th>' +
                        '<td>' + currency + ' ' + total_manday_cost + '</td>' +
                        '</tr>');
    ...
    .on('click', '.btn_view_project_site', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                //inspector_cost for the project
                $('#table_view_project > tbody:last-child').append('<tr style="background-color:lightgrey" class="proj_added_row">' +
                    '<th colspan="4"><h4>5. Inspection Cost </h4></th > ' +
                    '</tr>' +
                    '<tr class="proj_added_row">' +
                    '<th>Inspection Status: ' + response.inspection.inspection_status + '</th>' +
                    '<th>Manday = ' + response.inspection.manday + '</th>' +
