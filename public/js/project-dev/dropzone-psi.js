$(document).ready(function() {
    ...
    var myDZ = new Dropzone("div.file_upload_psi", {
        ...
        parallelUploads: 100,
        maxFiles: 100,
        acceptedFiles: 'application/pdf, .doc, .docx, .pub, .jpeg, .jpg, .png, .gif, .xls, .xlsx, .xlsm, .ppt, .pptx, .DOC, .DOCX, .PUB, .JPEG, .JPG, .PNG, .GIF, .XLS, .XLSX, .PPT, .PPTX',
...
function savePsiAsDraft() {
    ...
    $('.product_category').each(function(i, obj) {
        var val = $(this).val();
        form_data.append('product_category[]', val);
    });
    $('.brand').each(function(i, obj) {
        var val = $(this).val();
        brand.push(val);
