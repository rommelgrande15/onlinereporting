...
$target_dir = "upload/PP/$session_user_id/";


    mkdir("$target_dir", 0777);// Create directory if it does not exist
...
$Fname=$_POST['name'];
$stmt5=$handler->prepare("DELETE FROM `product_photos` where `file_name` = '$Fname' and status = '0' and photo_category ='PP'");
	if($stmt5->execute()){
		$msg = "Successfully";
	}else{
		$msg = "error";
