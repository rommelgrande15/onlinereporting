...
$target_dir = "upload/sub_upload/$session_user_id" . "_" . "PS_" . "$date_time/";


    mkdir("$target_dir", 0775);// Create directory if it does not exist
...
$Fname=$_POST['name'];
$stmt5=$conn->prepare("DELETE FROM `product_photos_sub` where `file_name` = '$Fname' and status = '0' and photo_category ='PS'");
	if($stmt5->execute()){
		$msg = "Successfully";
	}else{
		$msg = "error";
