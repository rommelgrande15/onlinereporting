$(document).ready(function() {
    ...
    $('#btn_product').click(function() {
        ...
        if (add_count_null >= 1) {
            ...
            content_clone.append('<div class="row"><div class="col-md-12">' +
                ...
                '<br/><button type="button" class="btn btn-danger btn-rm pull-right" style="margin-right:10px;"><i class="fa fa-times"></i> Remove Product</button>' +
                '</div>' +
                '</div>' +
                '</div>');
    ...
    $('body').on('click', '.btn-contact-person-supplier', function() {
        var client_id = $('#loading_client').val();
        $.ajax({
            url: '/getoneclientsbyclibooksupplier/' + client_id,
            type: 'GET',
            beforeSend: function() {
                $('.send-loading').show();
            },
            success: function(response) {
                console.log(response);
                if (response.message == "Nodata") {
                    $('#contact-person-modal-supplier').modal('show');
                    $('.send-loading').hide();
                } else {
                    $('#div_edit_more_fields_client').empty();
                    $.each(response.client_contact_list, function(i, element) {
