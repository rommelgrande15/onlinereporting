...
$('#form_edit_prod_spec .dz-preview').remove();
$('#form_edit_tech_details .dz-preview').remove();
$('#form_edit_art_work .dz-preview').remove();
$('#form_edit_shipping_mark .dz-preview').remove();
$('#form_edit_packing .dz-preview').remove();
$('#form_edit_photo_files .dz-preview').remove();
			...
			response.productphoto.forEach(element => {							
				...
				$('#id_edit_tech_details').val("UP");
					...
					for (i = 0; i < existing_files.length; i++) {
						...
						var new_src="http://ticapp.tk/js/dropzone/upload/"+cat+"/"+c_code+"/"+f_name;
						...
						dz.emit("addedfile", existing_files[i], "http://ticapp.tk/js/dropzone/upload/"+cat+"/"+c_code+"/");
                    	dz.emit("complete", existing_files[i], "http://ticapp.tk/js/dropzone/upload/"+cat+"/"+c_code+"/");
                    	if (ext == "pdf" || ext.indexOf("docx") != -1 || ext.indexOf("doc") != -1 || ext.indexOf("xlsx") != -1 || ext.indexOf("xls") != -1 || ext.indexOf("pptx") != -1 || ext.indexOf("ppt") != -1 || ext.indexOf("pub") != -1 || ext.indexOf("zip") != -1) {
                    	    dz.emit('thumbnail', existing_files[i], thumb_src)
                    	} else {
                    	    existing_files[i].previewElement.querySelector("img").src = new_src;
                    	}
                    	dz.files.push(existing_files[i], "http://ticapp.tk/js/dropzone/upload/"+cat+"/"+c_code+"/");
						}
					existing_files = [];
				}
