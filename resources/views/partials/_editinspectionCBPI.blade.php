...
<div class="row">
	...
	<div class="contact-select">
		<div class="col-md-4">
			<div class="form-group">
				...
				@if($inspection_details->client_book=='true')
					<select class="form-control client_select cli_required cli_draft_required" id="loading_client" name="loading_client" required disabled>
				@else
		...
		<div class="col-md-4">
			<div class="form-group">
				...
				{!! Form::text('loading_reference_number', $inspection_details->reference_number, ['class' => 'cli_required form-control cli_draft_required', 'id'=>'loading_reference_number']) !!}
			  </div>
		</div>
	</div>	
	...
	<div class="contact-select">
		...
		<div class="col-md-4">
			<div class="form-group">
				...
				{!! Form::text('loading_inspection_date_to', $inspection_details->inspection_date_to, ['class' => 'form-control cli_required inspection_date cli_draft_required']) !!}
			  </div>
		</div>
		...
		<div class="cbpi-clone-inspector-container">
			<div class="cbpi-clone-inspector">
      			<div class="col-md-6">
					<div class="form-group">
					    {!! Form::label('loading_inspector', 'Assign / Change Inspector') !!}
						...
						<input type="hidden" name="old_inspector_cbpi" id="old_inspector_cbpi" value="{{$inspection_details->inspector_id}}" disabled>
						<select class="cli_required form-control cli_draft_required sel-inspector" name="loading_inspector" id="loading_inspector" required>
	...
	<div class="col-md-6">
		<div class="form-group">
			{!! Form::label('loading_inspector_edit', '--Select Inspector Account for FM login details--') !!}

			<select class="form-control fm_inspector_cbpi_edit" name="fm_inspector_cbpi_edit" id="fm_inspector_cbpi_edit" required>
		...
		<input type="hidden" value="" id="fm_username_cbpi_edit">
		<input type="hidden" value="" id="fm_password_cbpi_edit">
	</div>
		<div class="col-md-12">
		...
		<div class="col-md-4">
				<div class="form-group">
					...
					{!! Form::text('cbpi_manday', $inspection_details->manday, ['class' => 'form-control  psi_required psi_draft_required cbpi_manday','required'=>'','readOnly'=>'true']) !!}
				</div>
		</div>
</div>
...
<div class="row">	
	...
	<div class="col-md-12">
							...
							<div class="form-group">
								...
								<label class="checkbox-inline">
									@if($inspection_details->project_type=="app_project")
										<input type="radio" name="project_type_cbpi" value="app_project" id="app_project_cbpi"  onclick="changeProjectTypeCbpi('app')" required checked> APP Project
									@else
										<input type="radio" name="project_type_cbpi" value="null" id="app_project_cbpi"  onclick="changeProjectTypeCbpi('app')" required> APP Project
									@endif
		...
		@if($inspection_details->project_type=="word_project")
			...
			@else
				<div class="col-md-4" id="div_template_word_cbpi">
					<div class="form-group">
						{!! Form::label('template_word', 'Select Template') !!}
						<select class="form-control cli_required" name="loading_template_word" id="loading_template_word" required>
		...
		@else
			@if($inspection_details->service=="site_visit")
			@else
				<div class="col-md-4" id="div_template_word_cbpi" style="display:none;">
					<div class="form-group">
						{!! Form::label('loading_template_word', 'Select Template') !!}
						<select class="form-control" name="loading_template_word" id="loading_template_word" required>
...
<div class="col-md-offset-9 col-md-3">
	...
	@if($inspect_stat_lower=='released')
		{!! Form::button('Save and Publish (No Email)', ['class' => 'btn btn-warning btn-block btn-cbpi-edit-submit-no-email','style'=>'margin-top: 5px;','type'=>'button', 'id'=>'CBPI_submit_no_email']) !!}	
	@endif
...
<script>
	...
	jQuery('#loading_inspection_date').on('change', function() {
        var client_code = $('#loading_client').val();
        var inspect_date_val = $('#loading_inspection_date').val();
        if (client_code != "") {
            $.ajax({
	...
	jQuery('#loading_client_old-function').on('change', function() {
