...
<div id="updateSupplierFactory" class="modal fade" role="dialog">
  ...
  <script>
	...
	function deleteFactoryCP(id){
	  ...
	  if (sure_delete) {
		...
		$.ajax({
		   type:'POST',
		   url:'/deletesupplierfactorycontact',
		   data:{
			_token: token,
			id:id
		  },
	  ...
	  return array_data;
	   ...
	   function updateFCPerson(id)
	   {
		  $.ajaxSetup({
			  ...
			  $.ajax({
				 type:'POST',
				 url:'/postupdatesupplierfactorycontactperson',
				 data:{
				   update_contact_person:update_contact_person,
				   update_contact_person_email:update_contact_person_email,
				   update_contact_person_number:update_contact_person_number,
