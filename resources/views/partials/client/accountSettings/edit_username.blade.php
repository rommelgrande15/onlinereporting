...
<script>
	jQuery(document).ready(function() {
		...
		$('#btn_update_username').click(function() {
			$('#add-show-error-change-account').hide();
			var dis = $(this);
			var add = $('.change_account');
			//var add_password = $('.password');
			var add_count_null = 0;
			var email = document.getElementById('change_username');
			...
			if (add_count_null == 0) {
				if (!filter.test(email.value)) {
					...
					$('#add-show-error-change-account').hide();
					$(this).text("Saving...");
					$('#btn_update_username').attr('disabled', true);
					savealldata();
				}
			} else {
				$('#add-show-error-change-account').show();
			}
		});
