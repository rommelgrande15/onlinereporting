{!!Form::open(['id'=>'new_inspection_form','data-parsley-validate'=>'', 'route'=>'client-editinspection-mrn','class'=>'form-inspection','enctype'=>'multipart/form-data'])!!}
	<div class="row">		
		<div class="col-md-12">
		...
		<input type="hidden" value="{{$inspection->reference_number}}" class="form-control reference_number" id="reference_number">
		<div class="col-md-4">
			<div class="form-group">
				{!! Form::label('client_project_number', 'Project Number') !!}
				{!! Form::text('client_project_number', $inspection->client_project_number, ['class' => 'form-control  client_project_number psi_required psi_draft_required','required'=>'','id'=>'client_project_number']) !!}
				{{-- {!! Form::text('client_project_number', $inspection->mrn_no, ['class' => 'form-control  client_project_number psi_required psi_draft_required','required'=>'','id'=>'client_project_number']) !!}
				{!! Form::text('client_project_number', $psiproducts, ['class' => 'form-control  client_project_number psi_required psi_draft_required','required'=>'','id'=>'client_project_number']) !!} --}}
				<br/><br/><br/>				
			</div>
		</div>
		<div class="col-md-4">
			<div class="form-group">			
				...
				{!! Form::text('inspection_date', $inspection->inspection_date, ['class' => 'form-control psi_required inspection_date psi_draft_required','required'=>'']) !!}
				<label title="Selecting this will not allow the factory to postpone or advance the inspection date">
					@if($inspection->factory_change_date=='yes')					
						<input type="checkbox" id="fac_change_date" name="fac_change_date" value="yes" checked> Do not allow factory to change this date.
					@else
		...
		<div class="col-md-4">
			<div class="form-group">
				...
				{!! Form::text('psi_shipment_date', $inspection->shipment_date, ['class' => 'form-control psi_required inspection_date psi_draft_required','required'=>'']) !!}
				<br/><br/><br/>
			</div>
		</div>
			...
			<div class="clone-inputs-contact-person">
				<div class="col-md-4">
					<div class="form-group">
						...
						{!! Form::select('contact_person', $client_contact, $get_cc->id, ['class' => 'form-control psi_required contact_persons psi_draft_required', 'placeholder'=>'Select Contact']) !!}
					</div>
				</div>
				<div class="col-md-4">
					<div class="form-group">
					    {!! Form::label('contact_person_number', 'Contact Telephone Number') !!}
					    {!! Form::text('contact_person_number', $get_cc->tel_number, ['class' => 'form-control psi_required numeric contact_number psi_draft_required','required'=>'','id'=>'contact_number']) !!}
				  	</div>
				</div>	
				<div class="col-md-4">
					<div class="form-group">
					    {!! Form::label('contact_person_email', 'Email Address') !!}
					    {!! Form::text('contact_person_email', $get_cc->email_address, ['class' => 'form-control psi_required contact_email psi_draft_required','id'=>'email_address']) !!}
				  	</div>
				</div>
	...
	<div class="row factory-select">
		...
		<div class="col-md-4">
			<div class="form-group">
				...
				{!! Form::select('supplier', $supplier_list, $inspection->supplier_id, ['class' => 'form-control supplier psi_required psi_draft_required', 'placeholder'=>'Select supplier',  'required'=>'']) !!}
		  	</div>
		</div>
		<div class="col-md-8">
			<div class="form-group">
				...
				<div class="group-body">
					<div class="row product-clone">
						...
						<div class="clone-inputs">
							<div class="row" style="margin-left:0.5px;">
									<div class="col-md-4">
										<div class="form-group">
											<label for="report_number">System Report Number</label>
												<input class="form-control report_number" readonly="true" name="report_number" type="text" id="system_reference_number" value="{{ $psi->reference_number }}">
										</div>
									</div>
								...
								<div class="row" style="margin-left:0.5px;">
									<div class="form-group" >										
											...
											<div class="input-group">
												{!! Form::select('product_name', $products, $psi->product_id, ['class' => 'form-control psi_required product_name psi_draft_required s_pname', 'placeholder'=>'Select Product']) !!}
							...
							<div class="row">
								<div class="col-md-12">
									<div class="form-group remove-product">
										<br>
										<button type="button" class="btn btn-danger btn-rm-perm pull-right" style="margin-right:10px;" data-id="{{$psi->id}}"><i class="fa fa-times"></i> Remove Product</button>
									</div>
								</div>
							</div>
						</div>
						@endforeach
						<input type="hidden" id="count-psi" value="{{$countpsi}}">
					</div>
				</div>
			</div>
		</div>
	...
	<div class="">
		<div class="col-md-offset-10 col-md-2">
			...
			@foreach ($inspection_mrn as $mrn)
			<input type="hidden" class="mrn_inspection_id" value="{{$mrn->inspec_id}}">
			@endforeach
			@foreach ($reports as $rep)
			<input type="hidden" class="report_id" value="{{$rep->id}}">
			@endforeach
...
function fillEPC(pc_dis,sub_pc_cl,sub_val){
	...
	//$.getJSON("http://ticapp.tk/json/categories.json", function(json) {
    //    $.each(json, function(i, optgroups) {
    //        $.each(optgroups, function(groupName, options) {
    //            if (groupName == cat_val) {
    //                $.each(options, function(j, option) {
    //                    var $option = $("<option>", {
    //                        text: option.text,
    //                        value: option.val
    //                    });
    //                    $option.appendTo(sub_pc_cl);
    //                });
    //            }
    //        });
	//	});
