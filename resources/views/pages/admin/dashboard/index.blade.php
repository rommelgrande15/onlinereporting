            ...
            <table id="inspections_table" class="table table-condensed cell-bor small dataTable no-footer">
                ...
                <tbody>
                    @foreach($inspections as $inspection)
                        @if($inspection->inspection_status=="Pending" || $inspection->inspection_status=="pending")
                            ...
                            @else
                                @foreach ($psiproduct as $psi_prod)                          
                                    @if($psi_prod->inspection_id==$inspection->id)   
                                        @if(is_int($psi_prod->product_name)) 
                        ...
                        <td class="text-center">{{$inspection->inspection_status}}</td>                                                   
                        <td class="text-center">
                            @if($inspection->service=="cbpi" || $inspection->service=="cbpi_serial" || $inspection->service=="cbpi_isce" || $inspection->service=="cli")
                                <a href="{{route('edit-project-cbpi',$inspection->id)}}" class="btn btn-info btn-xs" title="Edit Project Details"><i class="fa  fa-pencil"></i></a>
                            @elseif($inspection->service=="site_visit")
                            ...
                            @else
                                <a href="{{route('edit-project',$inspection->id)}}" class="btn btn-info btn-xs" title="Edit Project Details"><i class="fa  fa-pencil"></i></a>
                            @endif
                        </td>
...
@section('scripts')
	{!! Html::script('/js/admin/panel.js?v=2') !!}
@endsection
