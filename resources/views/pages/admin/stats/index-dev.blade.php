		...
		$.ajax({
			...
			success: function(response) {
				...
				if (datas) {
					...
					/*$.each(response.data_sets, function(key, value) {
						bg.push(value);
						//passed.push(value.passed);
						//failed.push(value.failed);
						//pending.push(value.pending);
					...
					chart.data.datasets = response.data_sets;
					chart.data.datasets.data = datas;
					console.log('date_sets: ' + bg);

					chart.options.title.text = 'Inspections Statistics ' + response.month + ' 2021';
					chart.update();
				} else {
					console.log('No data');
				}
