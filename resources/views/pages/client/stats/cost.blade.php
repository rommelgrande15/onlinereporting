...
@section('title','MD and Cost/day Statistics')
@section('page-title','MD and Cost/day Statistics')
    ...
    function search(chart, url, data) {
        ...
        $.ajax({
            ...
            success: function(response) {
                $('.send-loading').hide();
                datas = response.data;
                var total_manday = response.total_manday;
                //manday_computation(response.total_manday);
                if (total_manday <= 10) {
                    new_cost = 200;
                } else if (total_manday >= 11 && total_manday <= 19) {
                    new_cost = 190;
                } else if (total_manday >= 20 && total_manday <= 29) {
                    new_cost = 180;
                } else if (total_manday >= 30 && total_manday <= 39) {
                    new_cost = 170;
                } else {
                    new_cost = 160;
                }
                ...
                if (datas) {
                    ...
                    $.each(datas, function(key, value) {
                        ...
                        //total_cost.push(value.cost);
                        total_cost.push(new_cost * value.manday);

                        chart.data.labels = ranges;
                        chart.data.datasets[0].data = manday;
                        //chart.data.datasets[1].data = cost * manday;
                        chart.data.datasets[1].data = total_cost;

                    });
                    chart.options.title.text = 'MD and Cost(' + response.currency + ')/day Statistics ' + response.month + ' 2021';
                    chart.update();
